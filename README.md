...
* [Part 1: Set up your PC](#part-1-set-up-your-pc)
  ...
  * [Windows 10 / 11](#windows-10--11)
    ...
    * [Step 2. Install WSL2 and usbipd-win](#step-2-install-wsl2-and-usbipd-win)
    * [Step 3. Install the tools inside Ubuntu](#step-3-install-the-tools-inside-ubuntu)
    * [Step 4. How to attach the box to WSL2](#step-4-how-to-attach-the-box-to-wsl2)
* [Part 2: Install U-Boot onto the box](#part-2-install-u-boot-onto-the-box)
  * [Step 1. Download the files](#step-1-download-the-files)
  * [Step 2. Put the box into FEL mode](#step-2-put-the-box-into-fel-mode)
  * [Step 3. Install U-Boot](#step-3-install-u-boot)
* [Part 3: Boot Linux from a USB stick](#part-3-boot-linux-from-a-usb-stick)
  * [Step 1. Download the MiniArch image](#step-1-download-the-miniarch-image)
  * [Step 2. Write it to the USB flash drive](#step-2-write-it-to-the-usb-flash-drive)
  * [Step 3. Prepare the stick](#step-3-prepare-the-stick)
* [Part 4: Install onto the eMMC](#part-4-install-onto-the-emmc)
  * [Step 1. Boot the stick once](#step-1-boot-the-stick-once)
  * [Step 2. Boot standalone](#step-2-boot-standalone)
* [Part 5: First boot, WiFi and SSH](#part-5-first-boot-wifi-and-ssh)
  * [Step 1. Log in and set a password](#step-1-log-in-and-set-a-password)
  * [Step 2. See which networks are in range](#step-2-see-which-networks-are-in-range)
  * [Step 3. Connect](#step-3-connect)
  * [Step 4. Check it actually works](#step-4-check-it-actually-works)
  * [Step 5. SSH in from your PC](#step-5-ssh-in-from-your-pc)
  * [Step 6. Give it a fixed address, optional](#step-6-give-it-a-fixed-address-optional)
* [Part 6: Best practices after the install](#part-6-best-practices-after-the-install)
  * [Fix the clock first](#fix-the-clock-first)
  * [Repair the pacman keyring](#repair-the-pacman-keyring)
  * [Update, then install what you want](#update-then-install-what-you-want)
    * [Do not upgrade the kernel](#do-not-upgrade-the-kernel)
    * [Useful packages](#useful-packages)
    * [Three packages that do not exist for aarch64](#three-packages-that-do-not-exist-for-aarch64)
    * [If pacman times out downloading](#if-pacman-times-out-downloading)
  * [Firewall](#firewall)
    * [Point iptables at the legacy backend first](#point-iptables-at-the-legacy-backend-first)
    * [Then enable it](#then-enable-it)
  * [Optional tidying](#optional-tidying)
  * [The `wifi` command](#the-wifi-command)
  * [WiFi troubleshooting](#wifi-troubleshooting)
    * [The cold boot rule](#the-cold-boot-rule)
  * [Restoring stock Android](#restoring-stock-android)
  * [Troubleshooting](#troubleshooting)
* [How it works, and how this was worked out](#how-it-works-and-how-this-was-worked-out)
...
MiniArch is deliberately bare. It ships `curl`, `tar`, `xz`, `openssl` and
`systemctl`, but not `wget`, not `sudo`, not `less`, and no man pages. Every
package below was installed on the box and every command was run, so this list
is what worked rather than what ought to work.

Everything at once, if you just want a usable machine:

```bash
pacman -S nano vim htop btop tmux rsync tree unzip zip less man-db sudo git \
          bind nmap tcpdump ethtool socat traceroute whois iperf3 \
          fastfetch lsof strace usbutils pciutils ncdu parted dosfstools \
          cronie jq bc wget
```

That cost about 200 MB of the 8.7 GB free, and left 1.5 GB of RAM available.
...
|---|---|
| `nano`, `vim` | editors |
| `less` | a pager, which a lot of tools assume exists |
| `man-db` | `man`, since there are no man pages otherwise |
| `sudo` | not installed by default |
| `tmux` | a session that survives an SSH drop, which matters on a box whose only link is flaky WiFi |
| `htop`, `btop` | processes and resource use |
| `tree`, `ncdu` | see what is on the disk and what is eating it |
| `git`, `rsync`, `unzip`, `zip` | the usual |
| `jq`, `bc` | JSON and arithmetic in scripts |

**Network**

| Package | Gives you |
|---|---|
| `wget` | not installed by default, `curl` is |
| `bind` | `dig`, for when name resolution is the suspect |
| `iperf3` | measure throughput properly instead of guessing from `scp` |
| `tcpdump` | see what is actually on the wire |
| `nmap` | what is listening, here or elsewhere |
| `traceroute`, `whois` | routing and ownership |
| `ethtool` | link state |
| `socat` | forward a port, test a socket |

**Hardware and disks**

| Package | Gives you |
|---|---|
| `usbutils` | `lsusb`, which is how you confirm what is plugged in |
| `pciutils` | `lspci` |
| `parted`, `dosfstools` | partition and make FAT filesystems |
| `lsof`, `strace` | what has a file open, what a process is doing |

**Media previews**, only if you are running something that wants them:

| Package | Gives you |
|---|---|
| `ffmpeg` | video thumbnails, 12.7 MB download |
| `poppler` | `pdftoppm`, for PDF thumbnails |

**Scheduling**: `cronie` gives `crontab`. The service is not enabled until you
run `systemctl enable --now cronie`.

### Three packages that do not exist for aarch64

Checked with `pacman -Si`, all three are absent from the Arch Linux ARM repos:

| Package | Use instead |
|---|---|
| `p7zip` | `unzip` and `zip`, or build 7-Zip yourself |
| `bridge-utils` | `ip link` from `iproute2`, already installed |
| `neofetch` | `fastfetch`, which is packaged and is what replaced it upstream |

### If pacman times out downloading

On a link with packet loss, pacman fails with
`Resolving timed out after 10000 milliseconds` and rolls the whole transaction
back. Name resolution is usually the part that fails, not the download: on this
box two lookups in five took the full 5 second DNS retry while the rest
answered in 30 ms.

Resolve the mirror once and pin it, which skips DNS for the rest of the
install:

```bash
echo "$(getent ahostsv4 mirror.archlinuxarm.org | awk '{print $1; exit}') mirror.archlinuxarm.org" >> /etc/hosts
```

Then run the same `pacman -S` again. Anything already downloaded stays in
`/var/cache/pacman/pkg`, so each attempt gets shorter. Do not edit
`/etc/resolv.conf`, it is generated by `systemd-resolved` and your change will
be overwritten.
